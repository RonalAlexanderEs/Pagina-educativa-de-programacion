...
function addAlumno() {
  ...
  newRow.innerHTML = `
    ...
    <td>
      ...
      <button class="info-button" onclick="mostrarInformacion(this)">Información</button>
    </td>
  `;
  updateStats();
}
...
function mostrarInformacion(button) {
  ...
  const areaBeca = row.cells[1].innerText;
  const documentos = row.cells[2].innerText;
  const estado = row.cells[3].innerText;

  const infoTableBody = document.getElementById('infoTableBody');
  infoTableBody.innerHTML = `
    <tr><td><strong>Nombre del Estudiante:</strong> ${nombreEstudiante}</td></tr>
    <tr><td><strong>Área de Beca:</strong> ${areaBeca}</td></tr>
    <tr><td><strong>Documentos Adjuntos:</strong> ${documentos}</td></tr>
    <tr><td><strong>Estado:</strong> ${estado}</td></tr>
  `;
  ...
  fetch(`/api/beca/${solicitudId}`)
    ...
    .then(data => {
      ...
      modalDetails.innerHTML = `
        ...
        <p>Área de Beca: ${data.area_beca}</p>
        <p>Carta de Solicitud: <a href="${data.carta_solicitud}">Ver</a></p>
        <p>Comprobante: <a href="${data.comprobante}">Ver</a></p>
